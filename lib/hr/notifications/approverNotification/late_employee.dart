...
class Admin_Late_Leave_NotificationList extends StatefulWidget {
  ...
  Admin_Late_Leave_NotificationList({required this.xposition, required this.xstaff,required this.zemail});
  String xposition;
  String xstaff;
  String zemail;

  @override
  _Admin_Late_Leave_NotificationListState createState() => _Admin_Late_Leave_NotificationListState();
}

class _Admin_Late_Leave_NotificationListState extends State<Admin_Late_Leave_NotificationList> {
  // fetchnotification _noteList = fetchnotification();
  //fetchnotification _noteList = fetchnotification();
  ...
  Future<List<LateNotiModel>> fetchPost() async {
    ...
    if (response.statusCode == 200) {
      ...
      return parsed.map<LateNotiModel>((json) => LateNotiModel.fromJson(json)).toList();
    } else {
      throw Exception('Failed to load album');
    }
  }
                                ...
                                children: [
                                  FlatButton(
                                    ...
                                    onPressed: ()async{
                                      ...
                                      var response= await http.post(Uri.parse('http://172.20.20.69/api/adminapprove/lateapprove.php'),body:
                                      jsonEncode(<String, String>{
                                      "zid":"100060",
                                      "user":widget.zemail,
                                      "xposition":widget.xposition,
                                      "xstaff": snapshot.data![index].xstaff,
                                      "xyearperdate": "${snapshot.data![index].xyearperdate}",
                                      "xnote":"xnote",
                                      })
                                      );
                                        ...
                                        actions: [
                                          FlatButton(
                                            ...
                                            onPressed: ()async{
                                              ...
                                              var response= await http.post(Uri.parse('http://172.20.20.69/api/adminapprove/latereject.php'),body:
                                              jsonEncode(<String, String>{
                                                "zid":"100060",
                                                "user": widget.zemail,
                                                "xposition": widget.xposition,
